...
const AttackTable = ({ filter, intervalTime }) => {
    const intl = useIntl();
    const {
        run,
        data = [],
    ...
    useEffect(() => {
        ...
        const params = {
            vehicle_model_id: filter.vehicleModleId,
            pageIndex: 1,
            pageSize: 50,
    ...
    return <ScrollingTable loading={loading} columns={columns} dataSource={data} pagination={false} className={styles.attackTable} rowKey={(n, i) => i} scrollHeight={312} />;
};
