...
export const getStatistic = (params) => {
  return request.get('brandState/overview', { params }).then((res) => res.data);
};
// 获取攻击类型排行
export const getAttackTypeTop = (params) => {
  return request.get('brandState/attackType/top5', { params }).then((res) => {
    return res.data
...
export async function getThreatLevel(params) {
  return request.get('brandState/threatLevel', { params }).then((res) =>
    res.data
      .filter((v) => v.totalvalue != 0)
      .map((item) => ({
        ...item,
...
// 获取品牌趋势
export async function getVehicleTypeTop(params) {
  return request.get('brandState/vehicleModel/Top5', { params }).then((res) => {
    return res.data
      .filter((v) => v.totalvalue != 0)
      .map((item) => ({
        ...item,
...
export async function getAttackTrend(params) {
  return request.get('brandState/attackTrend', { params }).then((res) => {
...
export async function getAttackedCountryTop(params) {
  ...
  return request.get('brandState/attacked', { params }).then((res) =>
    res.data
      .filter((v) => v.totalvalue != 0)
      .map((item) => ({
        //...item,
...
export async function getSpareParts(params) {
  return request.get('brandState/device', { params }).then((res) => {
    return res.data
      .filter((v) => v.totalvalue != 0)
      .map((item) => ({
        ...item,
