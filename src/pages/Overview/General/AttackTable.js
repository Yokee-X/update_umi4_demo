...
import { FilterDayTypes, getEventData } from '@/pages/Overview/General/service';
...
const EventGrades ={}
const AttackTable = ({ filter, intervalTime, onChange }) => {
    const {
        run,
        data = [],
        loading: fetching
    } = useRequest(params => getEventData({ ...params, pageSize: 50 }), {
        manual: true,
    ...
    return <ScrollingTable loading={fetching} columns={columns} dataSource={data} pagination={false} className={styles.attackTable} rowKey={(n, i) => i} scrollHeight={310} />;
};
