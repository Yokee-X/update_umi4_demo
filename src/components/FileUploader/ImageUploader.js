...
class ImageUploader extends React.Component {
    ...
    render() {
        const { listType, maxFileCount, multiple, showAllowFileTypes, fileList } = this.props;
            ...
            return shouldRender ? (
                <div>
                    ...
                    <div className="ant-upload-text">
                        <FormattedMessage id={'Common.msg.upload'} />
                    </div>
                </div>
            ) : null;
        };
        return (
            <>
                <FileUploader {...this.props} onPreview={this.handlePreview} showUploadList showLoading={false} listType={listType === 'card' ? 'picture-card' : 'picture'}>
                    {renderAddButton()}
                </FileUploader>
                ...
                <Viewer visible={previewVisible} activeIndex={0} onClose={this.closePreview} images={[{ src: previewUrl }]} />
            </>
        );
    }
}
